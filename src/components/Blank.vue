    ...
    >
      ...
      <v-card>
        ...
        <v-card-text>
            ...
            <template v-for="(item, index) in saveItems">
              <template>
                <v-autocomplete
                    ...
                    item-text="NAME"
                    item-value="ID"
                    :messages="[`${item.machineBrand}/${item.machineModel}/${item.machineSerial}`]"
                    @input="getMachine(item)"
                    outlined
                    dense
                ></v-autocomplete>
                <v-file-input
                ...
                <template v-if="selectAction === 'Cнятие'">
                  ...
                  <template v-if="item.btns.length > 0">
                    <template v-for="(btn, index) in item.btns">
                      <div style="flex-direction: row; display: flex;"
                           :key="index + '/'+ item.machineID + '/' + index + '/'+'div'">
                        <v-text-field
                            :key="index + '/'+ item.machineID + '/' + index + '/'+'btnNumber'"
                            class="ma-2 pa-2"
                            v-model="btn.number"
                            label="Кнопка"
                            type="number"
                        ...
                        ></v-text-field>
                        <v-autocomplete
                            :key="index + '/'+ item.machineID + '/' + index + '/'+'btnRecipe'"
                            class="ma-2 pa-2"
                        ...
                        ></v-autocomplete>
                        <v-text-field
                            :key="index + '/'+ item.machineID + '/' + index + '/'+'quntity'"
                            class="ma-2 pa-2"
                            v-model="btn.cups"
                            label="Кол-во чашек"
                            outlined
                  ...
                  <v-btn class="ma-2 pa-2" @click="addBtn(item)" :key="index + '/'+ item.machineID + '/'+ 'btn'" small>
                    Добавить
                    кнопку с рецептом
                  </v-btn>
                </template>
                <template v-if="selectAction === 'Установка' || selectAction === 'Проверка'">
                  <v-autocomplete
                  ...
                  <template v-if="item.btns.length > 0">
                    <template v-for="(btn, index) in item.btns">
                      <div style="flex-direction: row; display: flex; align-items: baseline;"
                        ...
                        <v-col cols="1">
                          <v-btn icon @click="deleteBtn(item, btn)" :disabled="item.disabled">
                  ...
                  <template v-if="item.equips.length > 0">
                    <template v-for="(equip, indexEquip) in item.equips">
                      <div style="flex-direction: row; display: flex; align-items: baseline;"
                           :key="index + '/'+ item.machineID + '/' + indexEquip + '/'+'Equip/div'"
                      >
                        <v-autocomplete
                            :key="index + '/'+ item.machineID + '/' + indexEquip + '/'+'equips'"
                            class="ma-2 pa-2"
                            v-model="equip.ID"
                            ...
                            dense
                        ></v-autocomplete>
                        <v-btn icon @click="deleteEquip(item, equip)" :disabled="item.disabled">
                          <v-icon>mdi-window-close</v-icon>
                        </v-btn>
                      </div>
                    </template>
                  </template>
                  <v-btn class="ma-2 pa-2" @click="addBtn(item, item.btns.length + 1)"
                         :key="index + '/'+ item.machineID + '/'+ 'btn'" :disabled="item.disabled" small>
                    Добавить
                    кнопку с рецептом
                  </v-btn>
                  <v-btn class="ma-2 pa-2" @click="addEquips(item)" :key="index + '/'+ item.machineID + '/'+ 'equips'"
                         :disabled="item.disabled" small>
          ...
          <v-col cols="12">
            <v-btn v-if="selectAction === 'Установка'" small
                   @click="addRow">Добавить кофемашину
            </v-btn>
          </v-col>
        </v-card-text>
        <v-toolbar
  ...
  methods: {
    async getMachines() {
      try {
        ...
        while (isTrue) {
          ...
          this.machines.push(...list.data.result);
    ...
    async getMachine(item) {
      try {
        ...
        const selectMachine = await axios.post(this.url + 'lists.element.get', machineParams);
        console.log({selectMachine})
        item.machineSerial = selectMachine.data.result[0].PROPERTY_133 ? Object.values(selectMachine.data.result[0].PROPERTY_133)[0] : ''
        item.machineModel = selectMachine.data.result[0].PROPERTY_107 ? this.models[Object.values(selectMachine.data.result[0].PROPERTY_107)[0]] : ''
        item.machineBrand = selectMachine.data.result[0].PROPERTY_103 ? this.brands[Object.values(selectMachine.data.result[0].PROPERTY_103)[0]] : ''
        if (item.action === 'Установка') {
          ...
          const btns = []
          while (isTrue) {
            const listBtn = await axios.post(this.url + 'lists.element.get', paramsBtn);
    ...
    async saveInstallation(item, date, equipsIDs) {
      try {
        ...
        for (const btnIndex in item.btns) {
          ...
          batchParams.cmd[`btn_${btnIndex}`] = `lists.element.add?IBLOCK_TYPE_ID=lists&IBLOCK_ID=153&ELEMENT_CODE=btn_${btnIndex}/${date}&FIELDS[PROPERTY_609]=${item.btns[btnIndex].number}&FIELDS[PROPERTY_611]=${item.btns[btnIndex].recipeID}&FIELDS[PROPERTY_613]=${item.btns[btnIndex].cups}&FIELDS[PROPERTY_623]=${item.btns[btnIndex].oldCups}&&FIELDS[PROPERTY_615]=${item.machineID}&FIELDS[NAME]=Кнопка ${item.btns[btnIndex].number}`;
    ...
    async saveCheck(item) {
      try {
        ...
        for (const btnIndex in item.btns) {
          console.log(btnIndex);
          batchParams.cmd[`btn_${btnIndex}`] = `lists.element.update?IBLOCK_TYPE_ID=lists&IBLOCK_ID=153&ELEMENT_ID=${item.btns[btnIndex].id}&FIELDS[PROPERTY_609]=${item.btns[btnIndex].number}&FIELDS[PROPERTY_611]=${item.btns[btnIndex].recipeID}&FIELDS[PROPERTY_613]=${item.btns[btnIndex].cups}&FIELDS[PROPERTY_623]=${item.btns[btnIndex].oldCups}&FIELDS[PROPERTY_615]=${item.machineID}&FIELDS[NAME]=Кнопка ${item.btns[btnIndex].number}`;
          batchParams.cmd[`log_${btnIndex}`] = `lists.element.add?IBLOCK_TYPE_ID=lists&IBLOCK_ID=155&ELEMENT_CODE=log_${date}/log_${btnIndex}&FIELDS[NAME]=Лог&FIELDS[PROPERTY_625]=${item.machineID}&FIELDS[PROPERTY_627]=${item.btns[btnIndex].id}&FIELDS[PROPERTY_629]=${item.btns[btnIndex].cups}`
    ...
    async getObj() {
      ...
      const objData = await axios.post(this.url + 'lists.element.get', objParams);
      if (objData.data.result.length > 0) {
        ...
        for (const btnIndex in btns) {
          batchParams.cmd[`btn_${btnIndex}`] = `lists.element.get?IBLOCK_TYPE_ID=lists&IBLOCK_ID=153&ELEMENT_ID=${btns[btnIndex]}`;
          if (btnIndex > 49 || btnIndex == Number(btns.length) - 1) {
            ...
            for (const elemBatchIndex in batchRes) {
              item.btns.push({
                ...
                number: Object.values(batchRes[elemBatchIndex][0].PROPERTY_609)[0],
                recipeID: Object.values(batchRes[elemBatchIndex][0].PROPERTY_611)[0],
                cups: Object.values(batchRes[elemBatchIndex][0].PROPERTY_613)[0],
                oldCups: Object.values(batchRes[elemBatchIndex][0].PROPERTY_623)[0]
              })
            }
          }
        }
        ...
        for (const equipIndex in equips) {
          batchParams.cmd[`equip_${equipIndex}`] = `lists.element.get?IBLOCK_TYPE_ID=lists&IBLOCK_ID=143&ELEMENT_ID=${equips[equipIndex]}`;
