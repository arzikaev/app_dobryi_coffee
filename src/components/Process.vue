  ...
  <v-container class="flex-fill">
        ...
        <v-list>
          ...
          >
            <template v-for="(item, index) in filterItems">
              <v-list-item
                  :key="item.id"
                  @click="processOpen(item)"
              >
                <v-list-item-content>
                  <v-list-item-title v-text="item.name"></v-list-item-title>
                  <template v-for="(row,index) in item.descriptionRows">
                    <v-list-item-subtitle v-html="row.value" :key="index" v-if="index < 4"></v-list-item-subtitle>
                  </template>
                </v-list-item-content>
                <v-list-item-icon>
                      ...
                      <v-form v-if="!isDone">
                        <template>
                          <v-autocomplete
                              ...
                              item-text="NAME"
                              item-value="ID"
                              :messages="[`${item.obj.machineBrand}/${item.obj.machineModel}/${item.obj.machineSerial}`]"
                          ...
                          <template v-if="item.obj.btns.length > 0">
                            <template v-for="(btn, index) in item.obj.btns">
                              <div style="flex-direction: row; display: flex;"
                                   :key="index + '/'+ item.obj.machineID + '/' + index + '/'+'div'">
                                <v-text-field
                                    :key="index + '/'+ item.obj.machineID + '/' + index + '/'+'btnNumber'"
                                    class="ma-2 pa-2"
                                    v-model="btn.number"
                                    label="Кнопка"
                                    type="number"
                                ...
                                ></v-text-field>
                                <v-autocomplete
                                    :key="index + '/'+ item.obj.machineID + '/' + index + '/'+'btnRecipe'"
                                    class="ma-2 pa-2"
                                ...
                                ></v-autocomplete>
                                <v-text-field
                                    :key="index + '/'+ item.obj.machineID + '/' + index + '/'+'quntity'"
                                    class="ma-2 pa-2"
                                    v-model="btn.cups"
                                    label="Кол-во чашек"
                                    outlined
                          ...
                          <v-btn class="ma-2 pa-2" @click="addBtn(item)"
                                 :key="index + '/'+ item.obj.machineID + '/'+ 'btn'" small>
                            Добавить
                            кнопку с рецептом
                          </v-btn>
                        </template>
                        <v-row v-if="item.ShowComment === 'Y'" class="ma-1 pa-1 body-2">
                          <v-textarea class="body-2"
                        ...
                        <v-row class="ma-1 pa-1 body-2">
                          <v-col v-if="item.StatusOkLabel">
                            ...
                            >
                              {{ item.StatusOkLabel.split(' ')[0] }} <br>
                              {{ item.StatusOkLabel.split(' ')[1] }}
                            </v-btn>
                          </v-col>
                          ...
                          <v-col v-if="item.StatusCancelLabel" class="text-right">
                            <v-btn
                                :disabled="disabled"
                                x-large
                                color="red darken-3"
    ...
    <v-footer>
      <v-row align="center" class="ma-0">
        ...
        >
          <p>Процессов на стр.</p> {{ filterItems.length }}
        </v-col>
        <v-col cols="3">
          <v-select
              :items="pagesizes"
...
export default {
  ...
  methods: {
    ...
    searchItems() {
      if (this.search.length > 2) {
        this.filterItems = this.items.filter((item) => {
          return item.name.toLowerCase().indexOf(this.search.toLowerCase()) > -1 || item.description.toLowerCase().indexOf(this.search.toLowerCase()) > -1
        })
    ...
    async getProccess() {
      try {
        const user = await axios.post(this.url + 'profile', {auth: this.auth.ACCESS_TOKEN})
        ...
        for (const item of items) {
          console.log({item})
          const proccessData = await axios.post(this.url + 'lists.element.get', {
          ...
          const proccess = {
            ...
            comment: '',
            companyID: Object.values(proccessData.data.result[0].PROPERTY_275)[0],
            obj: {
              id: '',
              ibjID: '',
              machineID: '',
          ...
          if (item.PARAMETERS.StatusCancelLabel) proccess.StatusCancelLabel = item.PARAMETERS.StatusCancelLabel
          if (item.PARAMETERS.StatusOkLabel) proccess.StatusOkLabel = item.PARAMETERS.StatusOkLabel
          if (item.PARAMETERS.StatusNoLabel) proccess.StatusNoLabel = item.PARAMETERS.StatusNoLabel
          if (item.PARAMETERS.StatusYesLabel) proccess.StatusYesLabel = item.PARAMETERS.StatusYesLabel
      ...
      try {
        ...
        if (!isAlert) {
          ...
          for (const btnIndex in item.obj.btns) {
            ...
            batchParams.cmd[`log_${btnIndex}`] = `lists.element.add?IBLOCK_TYPE_ID=lists&IBLOCK_ID=155&ELEMENT_CODE=log_${date}/log_${btnIndex}&FIELDS[NAME]=Лог&FIELDS[PROPERTY_625]=${item.obj.machineID}&FIELDS[PROPERTY_627]=${item.obj.btns[btnIndex].id}&FIELDS[PROPERTY_629]=${item.obj.btns[btnIndex].cups}`
            if (btnIndex > 49 || btnIndex == Number(item.obj.btns.length) - 1) {
          ...
          const close = await axios.post(this.url + 'bizproc.task.complete', params)
          console.log({close})
